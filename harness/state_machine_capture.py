...
# --- WIRE TAP: capture exact request/response bytes for all OCI HTTP calls ---
# Patches requests.Session.send to intercept the final PreparedRequest
# (post-signing, post-prepare) and capture exact headers + body going on the wire,
# plus the response that comes back. Writes POST captures to JSON for comparison.
import requests as _requests
import hashlib as _hashlib

_original_send = _requests.Session.send
CAPTURE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "python_post_capture.json")
CAPTURE_ALL_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "python_all_requests.log")

def _get_body_bytes(prepared_request):
    """Extract body bytes from a PreparedRequest, handling various body types."""
    ...
    if hasattr(body, 'read'):
        ...
        data = body.read()
        if pos is not None and hasattr(body, 'seek'):
            body.seek(pos)
...
def _logging_send(self, request, **kwargs):
    ...
    if is_post:
        ...
        capture = {
            ...
            "body_sha256": _hashlib.sha256(body_bytes).hexdigest() if body_bytes else "",
            "timestamp": datetime.now().isoformat(),
        }
        print(f"[WIRE TAP] POST headers ({len(request.headers)} keys):", flush=True)
        for k in sorted(request.headers.keys()):
            v = request.headers[k]
            if k.lower() == 'authorization':
                v = v[:150] + '...[REDACTED]'
            print(f"[WIRE TAP]   {k}: {v}", flush=True)
        print(f"[WIRE TAP] POST body: {len(body_bytes)} bytes, SHA256={capture['body_sha256']}", flush=True)
        if body_bytes:
            print(f"[WIRE TAP] POST body preview: {body_bytes[:200].decode('utf-8', errors='replace')}", flush=True)

    # Send the request for real
    ...
    if is_post:
        try:
            ...
            print(f"[WIRE TAP] POST response body: {resp_body.decode('utf-8', errors='replace')[:300]}", flush=True)
            with open(CAPTURE_FILE, 'w') as f:
                json.dump(capture, f, indent=2)
            print(f"[WIRE TAP] Written to {CAPTURE_FILE}", flush=True)
        except Exception as e:
            capture["response_error"] = str(e)
            print(f"[WIRE TAP] Response capture error: {e}", flush=True)
            with open(CAPTURE_FILE, 'w') as f:
                json.dump(capture, f, indent=2)

    # Append to all-requests log
    with open(CAPTURE_ALL_FILE, 'a') as f:
        f.write(f"{datetime.now().isoformat()} {request.method} {request.url} -> {response.status_code}\n")
        if is_post:
            for k in sorted(request.headers.keys()):
                f.write(f"  {k}: {request.headers[k][:200]}\n")
            f.write(f"  body: {len(body_bytes)} bytes\n\n")

    print(f"[WIRE TAP] ---", flush=True)
    return response

_requests.Session.send = _logging_send

# CRITICAL: The OCI SDK uses a VENDORED copy of requests, not the standard module.
# We must patch oci._vendor.requests.sessions.Session.send as well.
try:
    import oci._vendor.requests.sessions as _oci_vendor_sessions
    _oci_vendor_sessions.Session.send = _logging_send
    print("[WIRE TAP] Patched oci._vendor.requests.sessions.Session.send", flush=True)
except ImportError:
    print("[WIRE TAP] oci._vendor not available (oci not imported yet)", flush=True)

# Also patch at the instance level as a fallback — override send on any
# Session instance the OCI SDK creates, by patching the base_client.
# This ensures even if the vendor import path is different, we catch it.
try:
    import oci.base_client as _oci_bc
    _orig_init = _oci_bc.BaseClient.__init__
    def _patched_init(self, *args, **kwargs):
        _orig_init(self, *args, **kwargs)
        if hasattr(self, 'session'):
            # Wrap the session's send method at the instance level
            _sess = self.session
            _orig_sess_send = _sess.send
            def _wrapped_send(request, **kw):
                return _logging_send(_sess, request, **kw)
            _sess.send = _wrapped_send
    _oci_bc.BaseClient.__init__ = _patched_init
    print("[WIRE TAP] Patched oci.base_client.BaseClient.__init__ for instance-level send", flush=True)
except ImportError:
    pass

print("[WIRE TAP] Wire tap installed", flush=True)
# --- END WIRE TAP ---
