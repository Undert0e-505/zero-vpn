...
The integrated Volunteer debug UI is separately gated:

```sh
./gradlew assembleDebug -PenableHevNative=true -PenableVolunteerDebug=true
```

`BuildConfig.VOLUNTEER_DEBUG_ENABLED` is true only for debug builds when the
property is supplied. Release builds hard-disable it.
