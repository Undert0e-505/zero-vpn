...
* Developer Mode-only.
* Integrated test UI also requires `BuildConfig.VOLUNTEER_DEBUG_ENABLED=true`
  from `-PenableVolunteerDebug=true`.
* No normal user UI.
...
* Browser validation remains the primary TUN-path proof because ZeroVPN is
  excluded from its own Volunteer VPN.
* UDP remains `udpMode=unsupported-not-validated-tor-socks`.
* UDP product status remains not supported.
